...
CMD_WITH_MEMORY="CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.run --nproc_per_node=2 --master-port 29500 ../inference/gpu_ollama_torchrun_planning.py \
...
CMD_WITHOUT_MEMORY="CUDA_VISIBLE_DEVICES=2,3 python -m torch.distributed.run --nproc_per_node=2 --master-port 29501 ../inference/gpu_ollama_torchrun_planning.py \
