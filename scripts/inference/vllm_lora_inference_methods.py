#!/usr/bin/env python3
"""
vLLM Server Inference Script for Multiple Planning Methods (CoT, ReAct, Memory)

This script connects to a running vLLM server with multiple LoRA adapters loaded
and generates conversations using different planning methods.

Prerequisites:
    Start vLLM server with all LoRA weights:
    ...
    python -m vllm.entrypoints.openai.api_server \
        ...
        --enable-lora \
        --lora-modules \
            10pct=/data50/shared_models/finetuned/llama-3.1-8b-10pct-lora \
...
Usage:
    python vllm_lora_inference_methods.py --lora_name 10pct --input_csv 10pct.csv --method cot
    python vllm_lora_inference_methods.py --lora_name 20pct --input_csv 20pct.csv --method react
    python vllm_lora_inference_methods.py --lora_name 40pct --input_csv 40pct.csv --method memory
"""
...
class VLLMClient:
    """Client for vLLM server inference with LoRA support"""

    def __init__(self, base_url: str = "http://localhost:8000", lora_name: str = "10pct"):
...
def main():
    parser = argparse.ArgumentParser(
        description="vLLM Server Inference for Conversation Generation (CoT, ReAct, Memory)",
...
Run all combinations:
    for method in cot react memory; do
        for pct in 10 20 40 60 80 100; do
            python vllm_lora_inference_methods.py --lora_name ${pct}pct --input_csv ${pct}pct.csv --method $method
        done
    done
        """
    )
    ...
    parser.add_argument("--lora_name", required=True,
                        help="LoRA adapter name as registered in server (e.g., 10pct, 20pct)")
    ...
    logger.info(f"LoRA: {args.lora_name}")
    ...
    # Initialize client
    client = VLLMClient(base_url=args.base_url, lora_name=args.lora_name)
    ...
    # Save output with method in filename
    csv_basename = os.path.splitext(os.path.basename(input_csv_path))[0]
    output_filename = f"llama31_8b_{csv_basename}_{args.method}_convos.csv"
