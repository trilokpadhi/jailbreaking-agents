#!/usr/bin/env python3
"""
vLLM Inference Script for Qwen3-30B-Instruct with LoRA Support

Usage:
    # Start vLLM server first (see launch_vllm_qwen.sh)
    python vllm_qwen_inference.py --input_csv convo_for_memory.csv
"""
...
class VLLMClient:
    ...
    def __init__(self, base_url: str = "http://localhost:8000", model_name: str = "qwen3-30b-instruct-merged"):
...
class OfflineVLLMEngine:
    """Offline vLLM engine for Qwen with LoRA support"""
    ...
    def __init__(
        ...
        logger.info(f"Loading Qwen base model from: {base_model_path}")
        if lora_path:
        ...
        if lora_path:
            self.lora_request = LoRARequest("qwen-ft", 1, lora_path)
    ...
    def _format_chat_messages(self, messages: List[Dict[str, str]]) -> str:
        ...
        for msg in messages:
            ...
            # Map system role if present in messages
            if role == "system":
...
def process_batch(
    ...
    # For offline mode, process sequentially
    if max_workers == 1:
        ...
        # For server mode, use thread pool
...
def main():
    parser = argparse.ArgumentParser(
        description="vLLM Inference for Qwen3-30B-Instruct with LoRA Support",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    # Server mode (base model)
    python vllm_qwen_inference.py --input_csv 10pct.csv

    # Server mode (with LoRA)
    python vllm_qwen_inference.py --use_lora --input_csv 10pct.csv

    # Offline mode (with LoRA)
    python vllm_qwen_inference.py --offline --use_lora --input_csv 10pct.csv
        """
    )
    ...
    parser.add_argument("--use_lora", action="store_true",
                        help="Use LoRA adapter (Qwen_ft)")

    # Mode selection
    parser.add_argument("--offline", action="store_true",
                        help="Use offline mode (load model directly)")
    ...
    parser.add_argument("--model_name", type=str, default=None,
                        help="Model name for server mode (uses server's registered name if not specified)")
    ...
    if not os.path.exists(args.base_model):
        logger.error(f"Base model does not exist: {args.base_model}")
        return
    ...
    if args.offline:
        ...
        # Use provided model name or auto-detect
        if args.model_name:
            model_name = args.model_name
        else:
            model_name = "qwen-ft" if args.use_lora else "/data50/shared_models/Qwen/Qwen3-30B-Instruct-LoRA-Merged"
        logger.info(f"Using model: {model_name}")
        engine = VLLMClient(base_url=args.base_url, model_name=model_name)
    ...
    lora_suffix = "_lora" if args.use_lora else "_base"
    output_filename = f"qwen3_30b_{csv_basename}{lora_suffix}_convos.csv"
