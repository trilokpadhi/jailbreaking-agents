...
llm_config = {
    "config_list": [
        {
            "model": "llama3.1",  # The model name you pulled
            "base_url": "http://localhost:11435/v1",  # Note the /v1 suffix
            "api_key": "ollama",  # Required but arbitrary value
            # "temperature": 0.7,
            "max_tokens": 512,
        }
...
def process_single_row(row):
    ...
    try:
        ...
        # pinxian is not filtering the data anymore
        # conversation = ast.literal_eval(row['agent3_output'])
        conversation = ast.literal_eval(row['agent3_output_converted']) # pinxian is not filtering the data anymore and using another agent
        ...
        v_agent = AssistantAgent(
            ...
            system_message=(
                ...
                "and your role is victim. Here is your past conversation {conversation}"
            ),
            # llm_config={"config_list": config_list},
            llm_config=llm_config,
        )
...
def main():
    parser = argparse.ArgumentParser(description="Run Bullying Simulation")
    parser.add_argument('--input_csv', required=True, help="Path to the input CSV file")
    parser.add_argument('--output_dir', required=True, help="Directory to save the output CSV file")
    parser.add_argument('--use_persona', action='store_true', help="Use persona data")
    args = parser.parse_args()

    input_csv = args.input_csv
    output_dir = args.output_dir
    user_persona = args.use_persona
    ...
    # # Process rows sequentially without parallelization for debugging
    # results = []
    # for _, row in tqdm(df.iterrows(), 
    #                    desc='Running Bullying Simulation over the dataset', 
    #                    total=len(df)):
    ...
    # Include date and time in the output file name
    output_csv_path = os.path.join(
        output_dir, 
        f"jailbreak_bullying_memory_{llm_config['config_list'][0]['model']}_{current_datetime}_{os.path.basename(input_csv)}"
    )
