...
FT_MODEL="./playground/deepseek-moe-merged"          # created by merge_lora.py
VLLM_PORTS=(8003 8004 8005)                           # one per GPU
GPUS=(0 1 2)
...
OUTPUT_DIR="generations_deepseek/"
...
start_vllm() {
    ...
    echo "=========================================="
    echo "Starting 3 vLLM servers for: ${model_tag}"
    echo "=========================================="
    VLLM_PIDS=()
    for i in 0 1 2; do
    ...
    echo "Waiting for all 3 servers to become ready (model loading may take a few minutes)..."
    for i in 0 1 2; do
        local ready=false
...
# ── 1. Base DeepSeek (no fine-tuning) ──────────────────────────────────────
MODEL_TAG="deepseek-base"
start_vllm "${BASE_MODEL}" "${MODEL_TAG}"
run_all_variants "${MODEL_TAG}"
stop_vllm

# ── 2. Fine-tuned DeepSeek ──────────────────────────────────────────────────
# Merge LoRA first if not done already
if [ ! -d "${FT_MODEL}" ]; then
    echo "Merging LoRA adapter into base model (one-time step)..."
    cd playground && python merge_lora.py && cd ..
fi

MODEL_TAG="deepseek-ft"
