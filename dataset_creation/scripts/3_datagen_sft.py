...
def datagen_sft(train_df, prompt_json_str, sft_conv_df):
    ...
    # df_final = pd.concat([df_train, final_conv], ignore_index=True)
    df_final = final_conv #100% of the data is toxic...
    df_final.to_csv("/home/tsutar3/HEART/data/sft_train_100_fixed.csv", index=True)
    # final_conv.to_csv("/home/tsutar3/HEART/data/sft_train_100.csv", index=True)

if __name__ == "__main__":
    df = pd.read_csv("/home/tsutar3/HEART/data/train.csv", index_col=0) # this is the sft dataframe
    json_file = '/home/tsutar3/HEART/data/tmp/ft_short_prompt.json' # this stores the shortened version of pinxian's prompts
    conv = pd.read_csv("/home/tsutar3/HEART/data/tmp/ft_conv_clean.csv") # Pinxian's dataset converted to sft dataset format...
